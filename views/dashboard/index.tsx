...
const CurrencyConverter: React.FC = () => {
  const [currencyTypes, setFromCurrency] = useState<String[]>([
    "USD",
    "EUR",
    "TRY",
  ]);
  ...
  const handleConvert = async () => {
    try {
      currencyTypes.forEach(async (currency) => {
        const response = await axios.get(`${apiUrl}/getCurrencyRates`);
  ...
  return (
    <div>
      ...
      <h3>USD/TRY: {currencyDatas.USDTRY}</h3>

      <h3>EUR/TRY: {currencyDatas.EURTRY}</h3>

      <h3>EUR/USD: {currencyDatas.EURUSD}</h3>
    </div>
  );
};
